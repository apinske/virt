#!/bin/sh
if [ ! -f alpine-minirootfs-3.14.0-$ARCH.tar.gz ]; then
    wget https://dl-cdn.alpinelinux.org/alpine/v3.14/releases/$ARCH/alpine-minirootfs-3.14.0-$ARCH.tar.gz
fi
...
mount vda.img mnt
cd mnt
tar xf ../alpine-minirootfs-3.14.0-$ARCH.tar.gz
