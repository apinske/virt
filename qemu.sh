...
sudo /usr/lib/qemu/virtiofsd --socket-path=/var/run/virtiofsd/fs0 -o source=$PWD -o log_level=warn &
...
sudo qemu-system-x86_64 \
      ...
      -chardev socket,id=fs0,path=/var/run/virtiofsd/fs0 -device vhost-user-fs-pci,queue-size=1024,chardev=fs0,tag=fs0 \
      ...
      -kernel vmlinuz -append "console=hvc0 root=/dev/vda quiet"
